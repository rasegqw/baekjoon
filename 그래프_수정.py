...
# # 위상 정렬을 위한 우선순위 큐 (최소값 우선, 작은 숫자부터)
# pq = []
# for i in range(1, N + 1):
#     if in_degree[i] == 0:
#         heapq.heappush(pq, i)  # 작은 숫자부터 방문

# result = [0] * (N + 1)
# order = 1  # 가장 작은 순서부터 할당
# count = 0  # 방문한 노드 수

# while pq:
#     node = heapq.heappop(pq)  # 가장 작은 번호 선택
#     result[node] = order
#     order += 1
#     count += 1  # 방문한 노드 수 증가
...
#             heapq.heappush(pq, neighbor)  # 작은 숫자 우선 처리
